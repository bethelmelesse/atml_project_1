...
class GNN(torch.nn.Module):
    def __init__(self, c_in, c_hidden, c_out, num_layers, dp_rate):
        ...
        layers += [GCNConv(in_channels, c_out, cached=True)]
        self.layers = torch.nn.Sequential(*layers)
        self.softmax = torch.nn.LogSoftmax()
        self.dropout = dp_rate
        self.initial_weight =  torch.nn.Linear(c_in, c_hidden)
        self.final_weight = torch.nn.Linear(c_hidden, c_out)
    ...
    def forward(self, x, adj_t):
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i == 0:
                x = self.initial_weight(x) + out
            elif x.size() == out.size():
                x = out + x
            else:
                x = self.final_weight(x) + out
