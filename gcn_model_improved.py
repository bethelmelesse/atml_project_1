...
class GNN(torch.nn.Module):
    def __init__(self, c_in, c_hidden, c_out, num_layers, dp_rate):
        ...
        for l_idx in range(num_layers-1):
            FC_layers.append(torch.nn.Linear(in_channels, in_channels))
            layers += [
                GCNConv(in_channels, out_channels)
            ]
            FC_layers.append(torch.nn.Linear(out_channels, out_channels))
            in_channels = c_hidden
        FC_layers += [torch.nn.Linear(in_channels, in_channels)]
        layers += [GCNConv(in_channels, c_out)]
        FC_layers += [torch.nn.Identity()]
        ...
        # self.conv1 = GCNConv(c_in, c_hidden, cached=True)
        # self.conv2 = GCNConv(c_hidden, c_out, cached=True)
        # self.dropout = dp_rate
    ...
    def forward(self, x, adj_t):
        ...
        for i in range(self.num_layers):
            out = self.FC_layers[2*i](x)
            out = F.relu(out)
            out = F.dropout(out, p=self.dropout, training=self.training)

            out = self.layers[i](out, adj_t)
            out = F.relu(out)
            out = F.dropout(out, p=self.dropout, training=self.training)

            out = self.FC_layers[2*i+1](out)
